...
async function init() {
  ...
  if (connectTimer.getTimeLeft() > 0 // 설정된 타이머의 남은 시간은 0ms 이하
  ...
  && abstController.requestConnectCount !== 1)  // connect() 요청 1회
    throw new Error();

  // 남아 있는 시간 만큼 대기. 새로이 돌아가고 있음
  await Promise.delay(abstController.connectTimer.getTimeLeft());
  // connectCount: 0,  disconnectCount: 1
  if (receiver.disconnectCount !== 1 && abstController.requestConnectCount !== 2) throw new Error('disconnectCount !== 1');
  ...
  if(abstController.requestConnectCount !== 2){
    throw new Error(`Expected abstController.requestConnectCount === 2, But ${abstController.requestConnectCount}`);
  }
  ...
  if (abstController.requestConnectCount !== 3) {
    throw new Error(`Expected abstController.requestConnectCount === 3, But ${abstController.requestConnectCount}`);
  }
  ...
  if (abstController.requestConnectCount  !== 3) {
    throw new Error(`Expected abstController.requestConnectCount  === 3, But ${receiver.requestConnectCount}`);
  }
  ...
  if (abstController.requestConnectCount !== 4) {
    throw new Error(`Expected abstController.requestConnectCount === 4, But ${abstController.requestConnectCount}`);
  }
  abstController.client = {
    alive: true
  };
